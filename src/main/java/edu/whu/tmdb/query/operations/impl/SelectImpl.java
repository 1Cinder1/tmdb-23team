...
//            这里是针对selectItem是表达式的判定，a或者a+2或者a+b*c都会被认定为表达式
    ...
    private void projectSelectExpression(SelectItem item, SelectResult entireResult,
        ...
        // 解析表达式获取所需的属性
        // 假设attributeParser返回的是表名和列名的组合
        ArrayList<String> tableColumn = new ArrayList<>();
        attributeParser(expression.toString(),tableColumn);
        // 执行表达式计算
        ArrayList<Object> dataList = (new Formula()).formulaExecute(expression, entireResult);
        ...
        for (int i = 0; i < dataList.size(); i++) {
            if (Objects.nonNull(projectResult.getTpl())) {
                Tuple tuple = projectResult.getTpl().tuplelist.get(i);
                tuple.tuple = Arrays.copyOf(tuple.tuple,tuple.tuple.length+1);
                tuple.tuple[tuple.tuple.length-1] = dataList.get(i);
            }else {
                Tuple tuple = entireResult.getTpl().tuplelist.get(i);
                // 为新的TupleList添加元组
                Tuple newTuple = resTupleList.tuplelist.get(i);
                newTuple.tuple = new Object[]{dataList.get(i)};
                newTuple.delete = false;
                newTuple.tupleId = tuple.tupleId;
                newTuple.tupleSize = 1;
                newTuple.classId = tuple.classId;
                newTuple.tupleIds = new int[newTuple.tupleSize];
                Arrays.fill(newTuple.tupleIds,newTuple.tupleId);
            }
        }
        TupleList tpl;
        if (Objects.isNull(projectResult.getTpl())) {
            projectResult.setTpl(resTupleList);
        }

        // select company.age+company.age as age2 from company
//        projectResult.getAttrname()[indexInResult] = tableColumn.get(1); // 更新字段名
//        projectResult.getType()[indexInResult] = "computed"; // 设定为计算类型或根据实际情况确定类型

//        if (alias.isUseAs()) {
//            projectResult.getAlias()[indexInResult] = alias.toString(); // 如果有别名，设置别名
//        } else {
//            projectResult.getAlias()[indexInResult] = tableColumn.get(1); // 否则别名和列名相同
//        }
        // 1.attrName赋值
